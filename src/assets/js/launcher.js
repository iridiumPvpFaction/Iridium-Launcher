...
const minecraft_folder = require('minecraft-folder-path');
        ...
        if (!accounts.length) {
            ...
            for (let account of accounts) {
                ...
                if (account.meta.type === 'AZauth') {
                    ...
                    let iridium_folder_path = minecraft_folder.replace("minecraft", "iridium");
                    iridium_folder_path =  iridium_folder_path + "/auth_infos.auth"
                    fs.writeFile(iridium_folder_path, refresh_accounts.access_token, (err) => {
                        if (err) {
                            console.error('Erreur lors de la sauvegarde du fichier : ' + err.message + " dans le dossier : " + iridium_folder_path, err);
                        } else {
                            console.log('Le texte a été sauvegardé avec succès dans' +  iridium_folder_path + " avec la valeur " + refresh_accounts.access_token);
                        }
                    });
